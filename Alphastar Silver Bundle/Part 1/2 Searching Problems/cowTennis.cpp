...
int main() {
    int N; cin >> N;
    v<ll> cows(N); rep(i,0,N) cin >> cows[i]; sort(all(cows));

    ll count = 0;
    rep(i,0,N) {
        rep(j,i+1,N) {
            ll dist = cows[j] - cows[i];
            int idxA = lower_bound(all(cows), cows[j]+dist)-cows.begin();
            int idxB = lower_bound(all(cows), cows[j]+2*dist+1)-cows.begin();
            count += (idxB - idxA);
        }
    }
